...
export const twitterCallback = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
  ...
  // Check if user needs to be created or updated in your system
  // This implementation depends on your user management approach
  let userData;
  try {
    // Find or create user based on Twitter ID
    userData = await userService.findOrCreateUserBySocialId('twitter', user.twitterId, {
      socialId: user.twitterId,
      username: user.username,
      displayName: user.displayName,
      email: user.email,
      profilePicture: user.profilePicture,
      twitterAccessToken: twitterTokens.access_token,
      twitterRefreshToken: twitterTokens.refresh_token
    });
  } catch (error) {
    console.error('Error finding/creating user:', error);
    // Continue without DB integration for now
    userData = { userId: 'twitter-user' };
  }
